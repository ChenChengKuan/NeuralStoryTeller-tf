...
class CapgenWrapper(object):
    ...
    def get_caption(self, image_loc, k=100):
        """
        ...
        feats = self.compute_features(self.model['net'], im).flatten()
        feats /= norm(feats)

        # Embed image into joint space
        feats = embedding.encode_images(self.model['vse'], feats[None,:])

        # Compute the nearest neighbours
        scores = numpy.dot(feats, self.model['cvec'].T).flatten()
        sorted_args = numpy.argsort(scores)[::-1]
        sentences = [self.model['cap'][a] for a in sorted_args[:k]]
